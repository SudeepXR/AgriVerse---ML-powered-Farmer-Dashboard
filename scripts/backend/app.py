...
from backend.shelf_life.predict import predict_shelf_life
from datetime import datetime, timedelta

# Database configuration
DATABASE = os.path.join(os.path.dirname(__file__), "database", "agriculture.db")
...
@app.route("/soil-report/latest/<int:farmer_id>", methods=["GET"])
def get_latest_soil_report(farmer_id):
    """
    Retrieves the most recent soil report for a given farmer.
    Used on page load to restore last generated report.
    """
    try:
        conn = get_db()
        cursor = conn.cursor()
        ...
        cursor.execute("""
            SELECT 
                nitrogen, phosphorus, potassium, 
                ph, temperature, humidity, rainfall, 
                soil_health_score, confidence_percentage, created_at
            FROM soil_reports 
            WHERE farmer_id = ? 
        ...
        if not row:
            return jsonify({"success": False, "message": "No previous report found"}), 200

        (
            N, P, K,
            ph, temperature, humidity, rainfall,
            health_score, confidence, created_at
        ) = row

        # Reuse SAME analysis logic as /nutrient
        N_status = nutrient_status(N, 50, 120)
        P_status = nutrient_status(P, 30, 80)
        K_status = nutrient_status(K, 30, 80)
        ...
        return jsonify({
            "success": True,
            "soil_analysis": {
                "nitrogen": {"status": N_status, "severity": N_sev, "value": N},
                "phosphorus": {"status": P_status, "severity": P_sev, "value": P},
                "potassium": {"status": K_status, "severity": K_sev, "value": K}
            },
            "soil_health_score": health_score,
            "recommendations": detailed_recommendations,
            "expected_yield_improvement": "Previously calculated",
            "confidence_percentage": confidence,
        ...
        return jsonify({
            "success": False,
            "error": f"Failed to fetch latest soil report: {str(e)}"
        }), 500

@app.route("/api/shelf-life/predict", methods=["POST"])
def shelf_life_prediction():
    data = request.get_json()

    required = ["farmer_id", "harvest_date", "transport_hours"]
    missing = [f for f in required if f not in data]
    ...
    if not farmer:
        return jsonify({"error": "Invalid farmer_id"}), 404

    # Decide storage location
    storage_district = data.get("storage_district", farmer["district"])

    if storage_district not in KARNATAKA_DISTRICTS:
        return jsonify({
            "error": "Unsupported storage district"
        }), 400

    storage_lat = KARNATAKA_DISTRICTS[storage_district]["lat"]
    storage_lon = KARNATAKA_DISTRICTS[storage_district]["lon"]

    harvest_date = datetime.strptime(
        data["harvest_date"], "%Y-%m-%d"
    )

    result = predict_shelf_life(
        harvest_date=harvest_date,
        storage_lat=storage_lat,
        storage_lon=storage_lon,
        transport_hours=float(data["transport_hours"])
    )

    return jsonify({
        "success": True,
        "crop": data.get("crop"),
        "storage_district": storage_district,
        "prediction": result
    })

# ==========================================================
# CROP RECOMMENDATION ML PROXY
# ==========================================================
VALID_SEASONS = ["Kharif", "Rabi", "Autumn", "Summer", "Whole Year"]
...
def crop_recommendation():
    """
    Proxies crop recommendation requests to the ML prediction service.
    Validates input, transforms to ML API format, and forwards the request.
    """
    data = request.get_json()

    # Validate region_id (required)
    region_id = data.get("region_id", "").strip() if data else ""
    if not region_id:
    ...
    # Validate soil_data (optional, but all-or-nothing)
    soil_data = data.get("soil_data")
    if soil_data:
        required_soil_fields = ["nitrogen", "phosphorus", "potassium", "ph"]
        missing_fields = [f for f in required_soil_fields if f not in soil_data]

        if missing_fields:
            return jsonify({
                "error": f"Partial soil data not allowed. Missing: {', '.join(missing_fields)}"
            }), 400
        ...
        try:
            ...
            return jsonify({"error": "Soil data values must be numeric"}), 400

        if not (0 <= nitrogen <= 300):
            return jsonify({"error": f"Nitrogen must be 0-300, got {nitrogen}"}), 400
        if not (0 <= phosphorus <= 300):
            return jsonify({"error": f"Phosphorus must be 0-300, got {phosphorus}"}), 400
        if not (0 <= potassium <= 300):
            return jsonify({"error": f"Potassium must be 0-300, got {potassium}"}), 400
        if not (0 <= ph <= 14):
            return jsonify({"error": f"pH must be 0-14, got {ph}"}), 400

        # Normalize soil_data with validated values
        soil_data = {
            "nitrogen": nitrogen,
    ...
    if season and season != "Whole Year":
        ml_payload["user_context"]["season"] = season

    if soil_data:
        ml_payload["soil_data"] = soil_data
    ...
    try:
        ...
        return jsonify({
            "error": f"ML prediction service error: {str(e)}"
        }), 503


# Add this route to your app.py file

@app.route('/api/farmer-details', methods=['GET'])
def get_farmer_details():
    farmer_id = request.args.get('farmer_id')
    if not farmer_id or farmer_id == 'undefined':
        return jsonify({'error': 'Valid Farmer ID is required'}), 400

    db = get_db()  # Use the existing database helper from your project
    
    # Query to fetch farmer details
    # This checks both the primary ID and the user_id link
    query = """
        SELECT 
            full_name, 
            username, 
    ...
    try:
        # executing using the standard flask-sqlite pattern found in your app
        cursor = db.execute(query, [farmer_id])
        farmer = cursor.fetchone()

        if farmer:
            # In your DB config, row_factory usually makes this behave like a dict
            # but if it returns a tuple, we access by index:
            return jsonify({
                'full_name': farmer[0],
                'username': farmer[1],
                'village': farmer[2],
