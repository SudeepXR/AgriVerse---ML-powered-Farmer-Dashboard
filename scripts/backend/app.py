...
try:
    ...
    print(f"WARNING: Disease classifier module could not be loaded: {e}")
...
try:
    ...
    print(f"WARNING: Sentiment analyser module could not be loaded: {e}")
    def analyze_crop(crop):
        return {"sentiment": "N/A", "analysis": "Module unavailable"}

try:
    from backend.price_predictor.app import predict
except Exception as e:
    print(f"WARNING: Price predictor module could not be loaded: {e}")
    def predict(district, commodity):
        return {"error": "Price predictor module unavailable"}

try:
    from backend.shelf_life_integration import shelf_life_service
except Exception as e:
...
def predict_shelf_life_route():
    if not shelf_life_service:
        return jsonify({"error": "Shelf life service unavailable"}), 503
    ...
    if not district or not harvest_date or transport_hours is None:
        return jsonify({"error": "Missing required fields: district, harvest_date, transport_hours"}), 400
        
    try:
        result = shelf_life_service.predict(district, harvest_date, float(transport_hours))
        return jsonify(result)
    except Exception as e:
